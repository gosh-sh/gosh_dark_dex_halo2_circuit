use halo2_proofs::{
    ...
    plonk::{create_proof, keygen_pk, keygen_vk, verify_proof, Error, Circuit},
...
pub fn generate_verififcation_key_without_witness<C: Circuit<Fr> + Default>(params: &ParamsKZG<Bn256>) -> VerifyingKey<G1Affine>{
    let circuit = C::default();
    keygen_vk(params, &circuit).unwrap()
}

pub fn generate_verififcation_key_without_witness_and_backup<C: Circuit<Fr> + Default>(params: &ParamsKZG<Bn256>, path: String) {
    let circuit = C::default();
