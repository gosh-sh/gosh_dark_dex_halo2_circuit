...
impl Circuit<Fr> for TestCircuit {
    type Config = SpongeConfig<Fr, Pow5Chip<Fr, 3, 2>>;
    ...
    fn synthesize(
        ...
        let chip =
            SpongeChip::<Fr, DEFAULT_STEP, Pow5Chip<Fr, 3, 2>>::construct(config, &self.0, self.1);
    ...
    let circuit = TestCircuit(
        PoseidonHashTable {
            inputs: vec![
                ...
                [
                    Fr::from_str_vartime("30").unwrap(),
                    Fr::from_str_vartime("1").unwrap(),
                ],
                [Fr::from_str_vartime("65536").unwrap(), Fr::zero()],
            ],
